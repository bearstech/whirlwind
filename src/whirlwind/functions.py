...
from datalib import fetchData, TimeSeries, timestamp
...
def asPercent(requestContext, seriesList, total=None):
    ...
    for series in seriesList:
        resultValues = [safeMul(safeDiv(val, totalVal), 100.0)
                        for val,totalVal in izip(series,totalValues)]
...
def _fetchWithBootstrap(requestContext, series, days=7):
    ...
    newSeries = TimeSeries(series.name, oldSeries.start, series.end,
                           series.step, newValues)
...
def holtWintersIntercept(alpha, actual,
                         last_season, last_intercept, last_slope):
...
def holtWintersAberration(requestContext, seriesList, delta=3):
    ...
    results = []
    for series in seriesList:
        confidenceBands = holtWintersConfidenceBands(requestContext,
                                                     [series], delta)
...
def smartSummarize(requestContext, seriesList, intervalString, func='sum', alignToFrom=False):
    ...
    for series in seriesList:
        ...
        newSeries = TimeSeries(newName, series.start, alignedEnd,
                               interval, newValues)
        newSeries.pathExpression = newName
        results.append(newSeries)
...
def summarize(requestContext, seriesList, intervalString, func='sum', alignToFrom=False):
    ...
    for series in seriesList:
        ...
        newName = "summarize(%s, \"%s\", \"%s\"%s)" % (series.name,
                                                       intervalString, func,
                                                       alignToFrom and
                                                       ", true" or "")
...
def hitcount(requestContext, seriesList, intervalString, alignToInterval = False):
    ...
    if alignToInterval:
        ...
        if interval >= DAY:
            ...
            requestContext['startTime'] = datetime(s.year, s.month,
                                                   s.day, s.hour)
        elif interval >= MINUTE:
            requestContext['startTime'] = datetime(s.year, s.month, s.day,
                                                   s.hour, s.minute)
    ...
    for series in seriesList:
        ...
        newSeries = TimeSeries(newName, newStart, series.end,
                               interval, newValues)
        newSeries.pathExpression = newName
        results.append(newSeries)
...
def events(requestContext, *tags):
    ...
    result_series = TimeSeries(name, start_timestamp, end_timestamp,
                               step, values, 'sum')
